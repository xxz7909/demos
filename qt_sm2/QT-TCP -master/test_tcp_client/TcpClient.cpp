    ...
    , ui(new Ui::TcpClient)
{
    ...
    QDir dir("/home/wang/client");
    if(!dir.exists())
    {
        qDebug()<< "client------exist";
        dir.mkpath("/home/wang/client");
    }
    ...
    QString desktop_dir = "/home/wang/client/" + app_name + ".desktop";
    QString exdir="/etc/xdg/autostart/" + app_name + ".desktop";
    QFileInfo fileinfo(exdir);
    if(!fileinfo.isFile())
    {
        QProcess p;
        QString strCmd=tr("echo -e '[Desktop Entry]\nName=%1\nName[zh_CN]=王的测试\nExec=/home/wang/tcp_client/test_tcp_client\nType=Application\nNoDisplay=false\n"
                          "X-GNOME-AutoRestart=true' > /home/wang/client/myApp.desktop").arg(name_last);
