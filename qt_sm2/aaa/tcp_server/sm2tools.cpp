...
VOID sm2Tools::InitializePrivateKey(DWORD KeySize,UCHAR *KeyBytes)
{
    if (m_lpsm2)
    {
        delete m_lpsm2;
    ...
    m_lpsm2=new sm2(KeySize,KeyBytes);
...
DWORD sm2Tools::Onsm2Encrypt(LPVOID InBuffer,DWORD InLength,LPVOID OutBuffer)
{
    //DWORD占4个字节
    DWORD OutLength=0,ExtraBytes = 0;
    if (m_lpsm2==NULL||OutBuffer==NULL)
    {
        return 0;
    }
    ...
    {
        //加密时，传入的数组必须时16个字节的
        m_lpsm2->Cipher(lpCurInBuff,lpCurOutBuff);
    ...
    if(leftnum)
    {
        ...
        m_lpsm2->Cipher(inbuff,lpCurOutBuff);
        lpCurOutBuff+=16;
        OutLength+=16;
    }
...
// 最近修改：2009 年 08 月 07 日
//------------------------------------------------------------------------------------------------------------
DWORD sm2Tools::Onsm2Uncrypt(LPVOID InBuffer,DWORD InLength,LPVOID OutBuffer)
{
    ...
    for(long i=0;i<blocknum;i++)
    {
        m_lpsm2->InvCipher(lpCurInBuff,lpCurOutBuff);
        lpCurInBuff+=16;
        lpCurOutBuff+=16;
        OutLength+=16;
    }
...
QByteArray sm2Tools::OpenFile(QString fileName){
    QFile file(fileName);
...
void sm2Tools::WriteFile(QString fileName, QByteArray data){
    QFile file(fileName);
...
//加密
void sm2Tools::FileEncryptor(QString inFileName,QString outFileName) {
    ...
    //调用smlib中的sm2.o进行sm2加密
    Onsm2Encrypt((LPVOID)mingwen, strlen(mingwen), (LPVOID)miwen); //进行加密
    //qDebug()<<miwen;
...
void sm2Tools::FileDecryptor(QString inFileName,QString outFileName){
    ...
    UCHAR *p = key;
    InitializePrivateKey(16, p); //进行初始化
    Onsm2Uncrypt((LPVOID)miwen, (DWORD)byteSize,(LPVOID)jiemi); //进行解密
