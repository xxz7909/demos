...
void Widget::acceptConnection()  //接受连接
{
    ...
    tcpClient.append(tcpServerConnection);


    connect(tcpServerConnection, SIGNAL(disconnected()), this, SLOT(disconnectedSlot()));

    QString ip = tcpServerConnection->peerAddress().toString();
            qint16  port = tcpServerConnection->peerPort();
            QString name=tcpServerConnection->peerName();
            QString temp = QString("%1:%2:%3").arg(name).arg(ip).arg(port);
...
void Widget::updateServerProgress() //接收数据
{
  ...
  if(bytesReceived <= sizeof(qint64)*2)
  {
       ...
       {  //接收文件名,并建立文件
           ...
           username=fileName.mid(k2+1,k3-k2);
   ...
   { //接收数据完成时
    ...
    filename_recived=tr("/home/wang/BYSJ_wang_main1.1/build-tcpReceiver-Desktop_Qt_5_9_0_GCC_64bit-Debug/%1").arg(fileName);
    filename_jiemi=tr("/home/wang/BYSJ_wang_main1.1/build-tcpReceiver-Desktop_Qt_5_9_0_GCC_64bit-Debug/info_decryptor_%1").arg(username);
...
void Widget::tabWidget_selected( )
{
    ...
    QString name=strl.at(1);
    QString ip = tcpServerConnection->peerAddress().toString();
    qint16  port = tcpServerConnection->peerPort();
    QString s = QString::number(port, 10);
    QString temp = QString("%1:%2:%3").arg(name).arg(ip).arg(s);

    //QString peer_str=strl.at(1)+':'+tcpServerConnection->peerAddress().toString()+':'+tcpServerConnection->peerPort();
    ...
    for(int i=0; i<tcpClient.length(); i++)
    {
        if(tcpClient[i]->state() == QAbstractSocket::UnconnectedState)
        {
            ...
            for(int j=0;j<row_num;j++)
            {
                ...
                qint16  port = tcpClient[i]->peerPort();
                QString s = QString::number(port, 10);
                if(peer==(tr("%1:%2")\
                        .arg(tcpClient[i]->peerAddress().toString())\
                        .arg(s)))
                {
...
void Widget::sortHeader(int index)
{
    if (index == 2)
    {
