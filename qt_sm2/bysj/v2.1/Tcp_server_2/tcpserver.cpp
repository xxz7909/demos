...
void Tcpserver::slotUpdateServer(int msgType,QString msg)
{
    ...
    else if(msgType == ISFILE)
    {
        ...
        qDebug()<<info_filename;
        if(info_filename.contains("/home"))
        {
            addlist();
        }
    }
...
void Tcpserver::addlist()
{
    QFile f(info_filename);
    ...
    //qint16  port = TcpClientSocket->peerPort();
    //QString s = QString::number(port, 10);
    QString temp = QString("%1:%2").arg(name).arg(strl.at(2));
